...
def opendb(host = dbHost, db = dbName):
    global dbHandle
    if not dbHandle is None:
        return
    #      raise SQLError('Database already open')
    if config.DATABASE_TYPE == 'MySQLdb':
      dbHandle = connect(host, db)
    else:
      dbHandle = connect(host)
      dbHandle.selectdb(db)
...
SQL-statements, die joins benötigen, sind damit nicht möglich.
...
"""
  ...
  def __init__(self, table, fields, WHERE = '', ORDER = ''):
    ...
    TABLES = "%(TABLES)s"
    self.sqlquerytemplate = "SELECT distinct %s\nFROM %s %s" % \
  ...
  def getUpdateSQL(self, values, where):
    ...
    sql = "UPDATE %s SET %s \nWHERE %s" % (self.table, setstr, where)
    return sql
...
##               VALUES (value [, value]** ) 
  ...
  def getInsertSQL(self, values):
    ...
    sql = "INSERT INTO %s ( %s ) \nVALUES ( %s )" % (self.table, colstr, valstr)
    return sql
