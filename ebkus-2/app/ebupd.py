...
def akteeinf(form):
  ...
  get_string_fields(akte, form, 
        ['vn', 'na', 'gb', 'ber', 'str', 'plz', 'ort', 'tl1', 'tl2', 'no'],'')
  ...
  if fall['bgy'] != today().year:
    jahresfallliste = FallList(where = 'bgy = %d' % fall['bgy'] + 
			     " and fn like '%%%s%%'" % stelle['code']) 
    fall['fn'] = str(len(jahresfallliste) + 1) + '-' + str(fall['bgy']) + stelle['code']
  else:
    fall['fn'] = check_str_not_empty(form, 'fn', "Keine Fallnummer")
  check_unique(fall['fn'], Fall, 'fn', "Fallnummer ist bereits vergeben")
  ...
  try:
    ...
    raise EE("Fehler beim Einfügen in die Datenbank: %s" % str(args))
...
def dokeinf(form):
  ...
  try:
    ...
    raise EE("Fehler beim Anlegen der Datei: %s" % str(args))
...
def removedoks(form):
  ...
  for d in dokids:
    try:
      ...
      raise EE("Fehler beim Loeschen, id: %s" % str(args))
...
##       dok['mtyp'] = cn('mimetyp', content_type)
    ...
    try:
      f = open('%s/%s' % (mk_akte_dir(akteold['id']), dok['fname']), 'w')
    except Exception, args:
      raise EE("Fehler beim Oeffnen der Datei. %s" % str(args))
    try:
      f.write(form['datei'].read())
    except Exception, args:
      raise EE("Fehler beim Speichern der Datei. %s" % str(args))
    try:
      f.close()
    except Exception, args:
      raise EE("Fehler beim Schliessen der Datei. %s" % str(args))
    try:
      akte_path = get_akte_path(akteold['id'])
    except Exception, args:
      raise EE("Fehler beim Finden des Dateipfades. %s" % str(args))
    try:
      os.chmod('%s/%s' % (akte_path, dok['fname']) ,0600)
    except Exception, args:
      raise EE("Fehler beim Anlegen der Datei: %s" % str(args))
...
def zdaeinf(form):
  ...
  if getQuartal(monat=zdadatum['month']) != getQuartal(monat='%(month)d' %today()) :
    raise EE("Fehler im z.d.A.-Datum: Monat '%d' " % zdadatum['month']
             + " ist nicht im laufenden %d. "
             % getQuartal(monat='%(month)d' %today())
             + "Quartal.")
...
def fseinf(form):
  ...
  jahresl = FachstatistikList(where = "fall_fn = '%s'" % fstat['fall_fn'] 
			      + ' and jahr = %d ' % fstat['jahr'])
  if len(jahresl) >= 1:
    raise EE("Fachstatistik für Fallnummer: '%s'" % fstat['fall_fn'] 
	     + " Jahr: '%d' " % fstat['jahr'] + " existiert bereits.")
  ...
  fstat['zm'] = check_code(form, 'zm', 'fszm', 
                            "Keine Zugangsart")
  fstat['ep'] = check_code(form, 'ep', 'fsep', 
                            "Kein Item zur Therapieerfahrung")
  fstat['gr'] = check_code(form, 'gr', 'fsgr', 
                            "Keine Geschwisterreihe")
  ...
  fstat['sd'] = check_code(form, 'sd', 'fssd', 
                            "Kein Item finanzielle Situation" )
  ...
  fstat['bg'] = check_code(form, 'bg', 'fsbg', 
  ...
  get_int_fields(fstat, form,
                 ['kkm', 'kkv',
                 'kki', 'kpa', 'kfa', 'ksoz', 'kleh', 'kerz', 'kson' ], 0)
...
def updfs(form):
  ...
  fstat['ep'] = check_code(form, 'ep', 'fsep', 
                            "Kein Item zur Therapieerfahrung", fstatold)
  fstat['gr'] = check_code(form, 'gr', 'fsgr', 
                            "Keine Geschwisterreihe", fstatold)
  fstat['hkm'] = check_code(form, 'hkm', 'fshe', 
  ...
  fstat['sd'] = check_code(form, 'sd', 'fssd', 
                            "Kein Item finanzielle Situation", fstatold)
  fstat['ba1'] = check_code(form, 'ba1', 'fsba', 
  ...
  fstat['bg'] = check_code(form, 'bg', 'fsbg', 
                            "Kein Beendigungsgrund", fstatold)
...
def jgheinf(form):
  ...
  if fallid == None or fallid == '' or fallid == 'None':
    ...
    check_unique(jghstat['fall_id'], Jugendhilfestatistik, 'fall_id',
                 "Fallid gibt es bereits")
  jghstat['mit_id'] = check_fk(form, 'mitid', Mitarbeiter,
                               "Kein Mitarbeiter")
  jghstat['fall_fn'] = check_str_not_empty(form, 'fall_fn',
                                           "Fallnummer fehlt") 
  check_unique(jghstat['fall_fn'], Jugendhilfestatistik, 'fall_fn',
               "Fallnummer gibt es bereits")
...
def gruppeeinf(form):
  ...
  check_unique(gruppe['gn'], Gruppe, 'gn',
  ...
  for m in mitid:
    m = check_unique(m, Mitarbeiter, 'id', "Mitarbeiter ID gibt es schon")
...
##      dok['mtyp'] = cn('mimetyp', content_type)
    ...
    try:
      f = open('%s/%s' % (mk_gruppe_dir(gruppeold['id']), dok['fname']), 'w')
    except Exception, args:
      raise EE("Fehler beim Oeffnen der Datei. %s" % str(args))
    try:
      f.write(form['datei'].read())
    ...
    try:
      ...
      raise EE("Fehler beim Setzen der Dateirechte: %s" % str(args))
  ...
  if form.has_key('fallid'):
    ...
    for f in fallid:
      try:
        ...
        raise EE("Fehler beim Löschen in der Datenbank: %s" % str(args))
...
def miteinf(form):
  ...
  mit['ben'] = check_unique(mit['ben'], Mitarbeiter, 'ben', 
...
def codeeinf(form):
  ...
  if form['mini'] != '' or form['maxi'] != '' or code['kat_code'] == 'dbsite':
...
def removeakten(form):
  ...
  faelle = FallList(where = 'zday <= %(loeschjahr)s and zdam <= %(loeschmonat)s and zday > 1'
                    % rmdatum )

  if len(faelle) < 1:
    raise EE('Keine Akten vorhanden, welche aelter als %(loeschmonat)s.%(loeschjahr)s sind.' % rmdatum )
  ...
  if len(gruppen) < 1:
    raise EE('Keine Gruppen vorhanden, welche aelter als %(loeschmonat)s.%(loeschjahr)s  sind' % rmdatum )
