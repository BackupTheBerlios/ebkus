...
def xcountitem(kat_code, d_list, d_item):
  ...
  values = map(lambda x, item = d_item: x[item], 
	       d_list)
  for c in codelist:
    i = values.count(c['id'])
    a = [c['name'], i, ((float(i)*100)/float(len(d_list)))]
    res.append(a)
  return res
...
def xcountnlist(nlist, d_list, d_item):
  """Liste mit Namen, Anzahl, Prozent fuer relat. Dictionarylisten
  der Fach- oder Jugendhilfestatistik.

  Anwendung: 
  anzahltermine = xcountnlist(mitarbeiterliste, fachstatliste, 'mit_id') """
  x = []
  res = []
  for d in d_list:
    x.append(d[d_item])
  for n in nlist:
    i = x.count(n['id'])
    a = [n['na'], i, ((float(i)*100)/float(len(d_list)))]
    res.append(a)
  return res
...
def fstat_ausgabe(res, feldliste, fstatl, mitarbeiterliste):
  """Ausgabe der Fachstatistik für die verschiedenen Abfragen
  """

  for f in feldliste:
    if f.get('kat_code'):
      res.append(thkategorie_t % f)
      if f['kat_code'] == 'fska' or f['kat_code'] == 'fskat': 
	for i in xcountbereich('%(kat_code)s' % f, fstatl, '%(feld)s' % f):
	  res.append(item_t % (i[0],i[1],i[2]))
      else:
	for i in xcountitem('%(kat_code)s' % f, fstatl, '%(feld)s' % f):
	  res.append(item_t % (i[0],i[1],i[2]) )
  ...
  for f in fstatl:
    for l in f['leistungen']:
      x.append(l) 
  res.append(thkategorie_t % {'name' : "Erbrachte Leistungen" } )
  for i in xcountitem('fsle', x, 'le'):
    res.append(item_t % (i[0],i[1],i[2]))
  res.append(tabende_t)

  x = []
  for f in fstatl:
    for l in f['fachstatelternprobleme']:
      x.append(l) 
  res.append(thkategorie_t % {'name' : "Problemspektrum der Eltern" } )
  for i in xcountitem('fspbe', x, 'pbe'):
    res.append(item_t % (i[0],i[1],i[2]))
  res.append(tabende_t)

  x = []
  for f in fstatl:
    for l in f['fachstatkindprobleme']:
      x.append(l) 
  res.append(thkategorie_t % {'name' : "Problemspektrum Kind/Jugendliche" } )
  for i in xcountitem('fspbk', x, 'pbk'):
    res.append(item_t % (i[0],i[1],i[2]))
  res.append(tabende_t)

  res.append(thkategorie_t % {'name' : "Mitarbeiter"} )
  for i in xcountnlist(mitarbeiterliste, fstatl, 'mit_id'):
    res.append(item_t % (i[0],i[1],i[2]))
  res.append(tabende_t)
  return res
