...
def dbexport(mit_id):
  ...
  this_export_dir = export_dir + '/export-%s-%s-%s' % \
  ...
  if site == config.MASTER_SITE:
    ...
    dump(ebapi.Fachstatistikkindproblem)
    dump(ebapi.Fachstatistikelternproblem)
  ...
  f = open('%s/export.inf' % this_export_dir, 'w')
...
def _dbimport(mit_id, dir, export_site, export_id, export_time):
  ...
  load(ebapi.Fachstatistikkindproblem)
  load(ebapi.Fachstatistikelternproblem)
...
def history():
  print "Oops. Jibt'et wohl noch nich"
