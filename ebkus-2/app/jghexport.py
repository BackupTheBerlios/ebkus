...
Die Datei wird in das Exportverzeichnis (gemäß config.EXPORT_DIR)
...
def get_laufende_nummer(r):
  ...
  MAX_LNR inkrementiert und in die Datenbankgeschrieben.
  MAX_LNR wird mit der höchsten vorhandenen laufenden Nummer
  in der Tabelle initialisiert.
  """
  global MAX_LNR
  lnr = r['lnr']
  if lnr is None or lnr < 1:           # Neue laufende Nummer wird benötigt
    if MAX_LNR is None:                # Maximum noch nicht festgestellt
      MAX_LNR = min(r._max('lnr'), 0)  # Falls alle Felder NULL (== None) sind, 
                                       # wird 0 genommen
    MAX_LNR = MAX_LNR + 1
...
def jghexport(jahr):
  ...
  export_dir = '%s/%s' (config.EBKUSHOME, config.EXPORT_DIR)
  filename = '%s/jgh_%s.txt' % (export_dir, jahr)
  f = open(filename, 'wb')
  opendb()
  jghl = ebapi.JugendhilfestatistikList (where = 'ey =  %s' % jahr)
...
if __name__ == '__main__':
  ...
  def check(cond):
    if not cond:
      raise 'Wrong, wrong, wrong!!!!'
  jahr = ''
  try:
    check(len(sys.argv) > 0)
    jahr = sys.argv[1]
    jahr = int(jahr)
    check(jahr > 1995 and jahr < 2500)
  except:
