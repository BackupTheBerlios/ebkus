...
def start(module_file, port=8080, threading=None):
	try_to_become_nobody()
	set_published_module(module_file,BoboRequestHandler)
	#server_address = ('138.201.10.164', port)
	...
	#funktioniert leider nicht. Socket ist oft noch besetzt.
	try:
	  httpd.serve_forever()
	except:
	  sys.exit(1)
...
if __name__=="__main__":
	import getopt
	#sys.path.insert(0,'/archive/soft/zope/Zope-1.9b1-src/lib/python')
	...
	for k,v in optlist:
		...
		if k=="-t":
			threading=1
	module_file=args[0]
	start(module_file,port,threading)
