...
class print_pdf(Request.Request):
  ...
  def processForm(self, REQUEST, RESPONSE):
    ...
    else: return  '<HTML><BODY>Kein Fall</BODY></HTML>'

    # Die Asciidatei in PDF-Format umwandeln und in 1 Datei schreiben.
...
class suchetxt(Request.Request):
  ...
  def processForm(self, REQUEST, RESPONSE):
    ...
    if len(expr) > 2:
      ...
      return '<HTML><BODY>Der Suchausdruck sollte mind. 3 Buchstaben haben</BODY></HTML>'
    ...
    if self.form.has_key('gruppeid'):
      ...
      header = {'titel': 'Suche in Texten',
                'ueberschrift':
                "Suchergebnis in den Texten der Gruppe &quot;%(name)s&quot; " % gruppe}
      ...
      if aktueller_fall:
        ...
        menue = menuedokzda_t % fall
        formhiddenv = formhiddennamevalues_t % ({'name' : 'fallid' ,
                                            'value' : letzter_fall['id']})

      header = {'titel': 'Suche in Texten',
                'ueberschrift':
                "Suchergebnis in der Akte &quot;%(vn)s %(na)s&quot; " % akte}
    ...
    res.append(dokausgabe7_t % ('<A name="suche">Suche in den Texten</A>', expr))
